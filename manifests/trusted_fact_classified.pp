...
#
# @param base_role_class The fallback role to use if no `pp_role` is set/avaiable
# @param warning If requested class does not exist we will print a warning message
#   informing the operator of this unless this parameter is set `false
  ...
  if $pp_role {
    if includable($pp_role) {
      ...
      if $warning {
        ...
        notify { "Unable to assign role `${pp_role}`: class does not exist - falling back to `${base_role_class}`":
          loglevel => warning,
        }
      }
